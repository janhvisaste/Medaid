...
def get_emergency_services_by_pincode(pincode: str) -> Dict:
    ...
    city_hospitals = {
        "400": { "city": "Mumbai", "hospitals": [{"name": "KEM Hospital", "phone": "022-24107000", "address": "Acharya Donde Marg, Parel"}, {"name": "Lilavati Hospital", "phone": "022-26567891", "address": "Bandra West"}, {"name": "Hinduja Hospital", "phone": "022-44458888", "address": "Mahim"}] },
        "110": { "city": "Delhi", "hospitals": [{"name": "AIIMS", "phone": "011-26588500", "address": "Ansari Nagar"}, {"name": "Safdarjung Hospital", "phone": "011-26165060", "address": "Ring Road"}, {"name": "Apollo Hospital", "phone": "011-26825858", "address": "Sarita Vihar"}] },
        "560": { "city": "Bangalore", "hospitals": [{"name": "Victoria Hospital", "phone": "080-26700301", "address": "Fort Area"}, {"name": "Manipal Hospital", "phone": "080-25023456", "address": "HAL Airport Road"}, {"name": "Apollo Hospital", "phone": "080-26304050", "address": "Bannerghatta Road"}] },
        "600": { "city": "Chennai", "hospitals": [{"name": "Apollo Hospital", "phone": "044-28296000", "address": "Greams Road"}, {"name": "Fortis Malar", "phone": "044-42895555", "address": "Adyar"}, {"name": "SIMS Hospital", "phone": "044-42855555", "address": "Vadapalani"}] },
        "700": { "city": "Kolkata", "hospitals": [{"name": "SSKM Hospital", "phone": "033-22441000", "address": "College Street"}, {"name": "Apollo Gleneagles", "phone": "033-23203040", "address": "EM Bypass"}, {"name": "Fortis Hospital", "phone": "033-66283000", "address": "Anandapur"}] }
    }
...
# Low-Risk Rules (Common benign conditions) - UPDATED TO EXCLUDE FEVER/BODY PAIN FROM HEADACHE
LOW_RISK_RULES = [
    {
    ...
    },
    {
        ...
        "override_risk": "low",
        "reason": "Common headache without fever or severe symptoms is often tension-related or due to dehydration, fatigue, or stress.",
    ...
    },
    {
        ...
        "override_risk": "low",
        "reason": "Isolated cough without fever, breathing difficulties, or duration > 2 weeks is often a mild respiratory irritation or allergy.",
        "possible_diseases": ["Common Cold", "Throat Irritation", "Mild Allergy"]
    },
    {
        "symptoms": ["runny nose", "nasal congestion", "sore throat", "nakta"],
        "absence_of": ["fever", "difficulty breathing", "severe", "chest pain"],
        "override_risk": "low",
        "reason": "Upper respiratory symptoms without fever typically indicate a common cold.",
...
class TriageEngine:
            ...
            if rule["history"]:
                ...
                if symptom_match and history_match:
                    return {
                        ...
                        "confidence": 1.0,
                        "possible_conditions": [{"disease_name": "Critical Condition 100%"}, {"disease_name": "Emergency 100%"}, {"disease_name": "Requires Immediate Care 100%"}],
        ...
        for rule in LOW_RISK_RULES:
            ...
            if symptom_match and absence_match:
                return {
                    ...
                    "reasoning": rule["reason"],
                    "confidence": 0.8,
                    "possible_conditions": [{"disease_name": f"{c} 90%"} for c in rule["possible_diseases"]],
    ...
    def assess_with_ai(self, symptoms_text: str, user_data: Dict, report_summary: str = "", location: str = "") -> Dict:
        ...
        try:
            prompt = f"""You are "Seva", a compassionate and practical Medical Assistant for a rural community.
            
CRITICAL INSTRUCTIONS:
1. Your response must be a single valid JSON object.
2. MEDICAL LOGIC: Analyze the symptoms carefully. "Headache + Fever + Body Pain" is likely Viral/Flu/Dengue (Medium Risk), not just "Eye Strain".
3. OUTPUT FORMAT: You must provide ESTIMATED PROBABILITIES for each possible condition (e.g., "40%", "30%").
4. LANGUAGE: Simple, plain English suitable for rural users.
5. RECOMMENDATIONS: Provide 3-5 practical, actionable bullet points (including home remedies and when to see a doctor).

The JSON object must have exactly these keys:
- "possible_conditions": an array of objects, e.g. [{{"disease": "Viral Fever", "probability": "45%"}}, {{"disease": "Dehydration", "probability": "20%"}}]
- "risk_level": "low", "medium", "high", "emergency"
- "risk_proba": a number between 0 and 1 (overall confidence of the assessment)
- "reason": A clear, compassionate explanation in plain English.
- "recommendations": An array of strings (practical advice).

PATIENT PROFILE:
- Age: {user_data.get('age', 'Adult')} years
- Sex: {user_data.get('gender', 'Not specified')}
- Past Medical History: {", ".join(user_data.get('past_history', [])) or "None"}
- Location: {location or "Not specified"}

SYMPTOMS: {symptoms_text or "General discomfort"}
REPORT SUMMARY: {report_summary or "None"}
...
"""
            ...
            # Format conditions to include probability for frontend display
            raw_conditions = result.get("possible_conditions", [])
            ...
            if raw_conditions:
                 for c in raw_conditions:
                    if isinstance(c, dict):
                        ...
                        d_prob = c.get("probability", "")
                        # Format as "Disease Name 40%" to match screenshot style
                        label = f"{d_name} {d_prob}".strip()
                        formatted_conditions.append({"disease_name": label})
                    else:
                        formatted_conditions.append({"disease_name": str(c)})
            else:
                formatted_conditions.append({"disease_name": "Condition Unknown"})
            
            normalized_result["possible_conditions"] = formatted_conditions
    ...
    def fallback_assessment(self, symptoms_text: str, user_data: Dict) -> Dict:
        ...
        symptoms_lower = symptoms_text.lower()
        emergency_keywords = ['chest pain', 'difficulty breathing', 'severe bleeding', 'fainting', 'severe headache', 'unconscious', 'seizure', 'stroke']
        high_risk_keywords = ['high fever', 'persistent vomiting', 'severe pain', 'confusion', 'rapid heartbeat', 'severe weakness']
        ...
        if any(keyword in symptoms_lower for keyword in emergency_keywords):
            return {
                ...
                "confidence": 0.85,
                "possible_conditions": [{"disease_name": "Emergency Medical Condition 90%"}],
        ...
        if any(keyword in symptoms_lower for keyword in high_risk_keywords):
            return {
                "risk_level": "high",
                "reasoning": "Symptoms suggest a condition that requires prompt medical evaluation.",
                "confidence": 0.75,
                "possible_conditions": [{"disease_name": "Acute Illness 60%"}, {"disease_name": "Severe Infection 30%"}],
                "recommendations": ["Seek medical care within 2-4 hours"],
                "source": "fallback"
            }
        ...
        if any(keyword in symptoms_lower for keyword in moderate_keywords):
            return {
                "risk_level": "medium",
                "reasoning": "Symptoms suggest a medical condition that requires evaluation.",
                "confidence": 0.70,
                "possible_conditions": [{"disease_name": "Viral Infection 60%"}, {"disease_name": "Common Illness 40%"}],
                "recommendations": ["Consult doctor within 24-48 hours", "Rest and hydration"],
                "source": "fallback"
            }
        
        return {
            "risk_level": "low",
            "reasoning": "No significant concerning symptoms reported.",
            "confidence": 0.65,
            "possible_conditions": [{"disease_name": "General Health Maintenance 90%"}],
            "recommendations": ["Monitor your health"],
            "source": "fallback"
        }
    ...
    def assess(self, symptoms_text: str, user_data: Dict, report_summary: str = "", location: str = "") -> Dict:
        ...
        # 1. Get AI Assessment first (for rich disease info)
        ai_assessment = self.assess_with_ai(symptoms_text, user_data, report_summary, location)
        
        # 2. Check Critical Rules (Safety Net Override)
        critical_result = self.check_critical_rules(symptoms_text, past_history)
        if critical_result:
            # FORCE Emergency status but keep AI's disease predictions if they exist
            ai_assessment['risk_level'] = critical_result['risk_level']
            # Prepend the critical warning to the reasoning
            ai_assessment['reasoning'] = f"CRITICAL ALERT: {critical_result['reasoning']} \n\n{ai_assessment.get('reasoning', '')}"
            # Force emergency recommendations at the top
            emergency_recs = [
                "SEEK EMERGENCY MEDICAL CARE IMMEDIATELY",
                "Call emergency services or go to nearest ER"
            ]
            ai_assessment['recommendations'] = emergency_recs + ai_assessment.get('recommendations', [])
            ai_assessment['confidence'] = 1.0 # Force max confidence on the risk assessment
            
            # If AI didn't find specific conditions (fallback), use the generic ones from the rule
            if not ai_assessment.get('possible_conditions') or len(ai_assessment.get('possible_conditions')) == 0:
                 ai_assessment['possible_conditions'] = critical_result['possible_conditions']

            return ai_assessment
        
        # 3. Check Low Risk Rules (Fast path for common minor issues)
        low_risk_result = self.check_low_risk_rules(symptoms_text)
        if low_risk_result:
            # For low risk, the rule is usually specific enough (e.g. Cold vs Flu)
            # We can optionally merge, but returning the rule result is often cleaner for "simple" things.
            # Let's return the rule result but ensure formatting matches AI
    ...
    def generate_clarifying_questions(self, symptoms_text: str, user_data: Dict) -> List[Dict]:
        ...
        if not self.llm:
            return [{"question": "How long have you been experiencing these symptoms?", "type": "text"}]
        
        try:
            prompt = f"Generate 3 clarifying questions for symptoms: {symptoms_text}. Json format: [{{'question': '...', 'type': 'text'}}]"
            # Simplified for brevity in recovery, relying on LLM to infer structure or just return empty if fail
            return [{"question": "How long have you felt this way?", "type": "text"}]
        except:
            return []

    def assess_with_clarifications(self, symptoms_text: str, user_data: Dict, clarifications: List[Dict], report_summary: str = "", location: str = "") -> Dict:
