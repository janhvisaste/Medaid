...
def generate_assessment_pdf(triage_record, user, user_profile=None):
    ...
    # A degraded assessment (AI could not complete it) is stored on the record's
    # similar_cases JSON. This PDF may be read or shared with no app UI around
    # it, so a degraded result must announce itself on the document itself,
    # prominently - not be inferred from an empty conditions section. Older
    # records only carried 'degraded'; fall back to it.
    _similar = getattr(triage_record, 'similar_cases', None) or {}
    is_degraded = bool(_similar.get('is_degraded') or _similar.get('degraded'))
    ...
    # Prominent degraded notice, placed at the very top so it is unmissable on
    # a shared/printed copy. Rendered as a bordered, filled callout box rather
    # than body text or a footnote. No emoji: reportlab's core fonts cannot
    # render them, and a missing glyph would undermine the warning.
    if is_degraded:
        degraded_notice_style = ParagraphStyle(
            'DegradedNotice',
        ...
        degraded_para = Paragraph(
            "REQUIRES CLINICIAN REVIEW: This assessment could not be completed by AI "
            "and requires review by a healthcare professional. No condition differential "
            "was generated. Treat the results below as incomplete general guidance, not a "
            "diagnosis.",
            degraded_notice_style,
        )
        degraded_box = Table([[degraded_para]], colWidths=[6.5 * inch])
        degraded_box.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, -1), colors.HexColor('#fff3cd')),
            ('BOX', (0, 0), (-1, -1), 1.5, colors.HexColor('#e0a800')),
            ('TOPPADDING', (0, 0), (-1, -1), 10),
            ('BOTTOMPADDING', (0, 0), (-1, -1), 10),
            ('LEFTPADDING', (0, 0), (-1, -1), 12),
            ('RIGHTPADDING', (0, 0), (-1, -1), 12),
        ]))
        elements.append(degraded_box)
    ...
    if user_profile:
        ...
        if user_profile.user.phone_number:
            patient_data.append(['Phone:', user_profile.user.phone_number])
    ...
    if conditions:
        ...
        # Never leave the differential a silent blank on a degraded report:
        # state explicitly that none was produced, paired with the notice above.
        elements.append(Paragraph("POSSIBLE CONDITIONS", styles['SubHeading']))
        elements.append(Paragraph(
            "No condition differential was generated. This assessment could not be "
            "completed by AI and requires review by a healthcare professional.",
            styles['Normal'],
        ))
        elements.append(Spacer(1, 20))
