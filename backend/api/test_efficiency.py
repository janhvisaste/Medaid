...
class HealthPassportEfficiencyTest(TestCase):
    ...
    def test_get_health_passport_queries(self):
        ...
        Usually it is around 7 queries: UserProfile + pagination COUNT + TriageRecords page +
        Conditions + MedicalReports + MedicalTests + AbnormalResults. The pagination COUNT is
        reused for total_consultations, so no separate count query is fired.
        Without prefetch, it would be 4 + 2 * (number of records/reports), which is N+1.
        We expect a constant 7 queries regardless of how many records exist.
        """
        # UserProfile, pagination COUNT, TriageRecord page, PossibleCondition,
        # MedicalReport, MedicalTest, AbnormalResult
